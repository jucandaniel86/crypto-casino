...
const getDepositWallets = async (): Promise<void> => {
  loading.value = true
  const data: any = await $fetch('/json/responses/withdraw.json')
  const DepositContainer = data.find((el: any) => el.container === 'WithdrawContainer')

  if (DepositContainer) {
    depositData.value = DepositContainer
    wallets.value = DepositContainer.data.withdrawCurrencies
  }
  loading.value = false
}
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <div v-else>
        <wallet-deposit-item label="Withdraw crypto" :wallets="cryptoWallets" />
        <wallet-deposit-item label="Withdraw fiat" :wallets="fiatWallets" />
      </div>
    </template>
  </div>
</template>
